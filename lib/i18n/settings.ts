...
export const settingsZhTW = {
  settings: {
    ...
    apiKey: 'API金鑰',
    apiKeyDesc: '配置你的API金鑰',
    apiBaseUrl: 'API端點地址',
    apiBaseUrlDesc: '配置你的API端點地址',
    ...
    testConnection: '測試連線',
    testConnectionDesc: '測試當前API配置是否可用',
    testing: '測試中...',
    agentSettings: '智慧體設定',
    ...
    maxTurns: '最大討論輪數',
    maxTurnsDesc: '智慧體之間最多討論多少輪（每個智慧體完成動作並回復算一輪）',
    priority: '優先順序',
    actions: '動作',
    ...
    // Provider settings
    providers: '語言模型',
    addProviderDescription: '新增自定義模型提供方以擴充套件可用的AI模型',
    providerNames: {
      openai: 'OpenAI',
      anthropic: 'Claude',
      google: 'Gemini',
    ...
    resetToDefault: '重置為預設配置',
    resetToDefaultDescription: '將模型列表恢復到預設狀態（保留 API 金鑰和 Base URL）',
    resetConfirmDescription:
      '此操作將清除所有自定義模型，恢復到內建的預設模型列表。API 金鑰和 Base URL 將被保留。',
    confirmReset: '確認重置',
    resetSuccess: '已成功重置為預設配置',
    saveSuccess: '配置已儲存',
    saveFailed: '儲存失敗，請重試',
    ...
    providerMetadata: 'Provider 後設資料',
    // Model editing
    editModel: '編輯模型',
    ...
    saveModel: '儲存',
    modelsManagementDescription:
      '在此管理該提供方的模型列表。若需選擇使用的模型，請前往"常規設定"。',
    // General settings
    howToUse: '使用說明',
    step1ConfigureProvider:
      '前往"模型提供方"頁面，選擇或新增一個提供方，配置連線資訊（API 金鑰、Base URL 等）',
    step2SelectModel: '在下方"使用模型"中選擇要使用的模型',
    step3StartUsing: '儲存設定後，系統將使用您選擇的模型',
    activeModel: '使用模型',
    activeModelDescription: '選擇當前用於 AI 對話和內容生成的模型',
    selectModel: '選擇模型',
    searchModels: '搜尋模型',
    noModelsFound: '未找到匹配的模型',
    noConfiguredProviders: '暫無已配置的提供方',
    configureProvidersFirst: '請先在左側"模型提供方"中配置提供方連線資訊',
    currentlyUsing: '當前使用',
    // TTS settings
    ttsSettings: '語音合成',
    // ASR settings
    ...
    ttsVoiceConfigHint: '每個 Agent 的音色可在首頁「課堂角色配置」中設定',
    enableASR: '啟用語音識別',
    ...
    enterCustomBaseUrl: '輸入自定義 Base URL',
    browserNativeNote: '瀏覽器原生 ASR 無需配置，完全免費',
    // Audio provider names
    providerOpenAITTS: 'OpenAI TTS (gpt-4o-mini-tts)',
    providerAzureTTS: 'Azure TTS',
    providerGLMTTS: 'GLM TTS',
    ...
    providerMinerU: 'MinerU',
    browserNativeTTSNote: '瀏覽器原生 TTS 無需配置，完全免費，使用系統內建語音',
    testTTS: '測試 TTS',
    testASR: '測試 ASR',
    testSuccess: '測試成功',
    testFailed: '測試失敗',
    ...
    browserTTSNoVoices: '當前瀏覽器沒有可用的 TTS voice',
    microphoneAccessDenied: '麥克風訪問被拒絕',
    microphoneAccessFailed: '無法訪問麥克風',
    ...
    qwenVoiceArthur: '被歲月和旱菸浸泡過的質樸嗓音',
    qwenVoiceNini: '糯米餈一樣又軟又黏的嗓音，那一聲聲拉長了的"哥哥"',
    ...
    qwenVoiceDylan: '北京胡同里長大的少年',
    qwenVoiceLi: '耐心的瑜伽老師',
    qwenVoiceMarcus: '面寬話短，心實聲沉——老陝的味道',
    qwenVoiceRoy: '詼諧直爽、市井活潑的臺灣哥仔形象',
    ...
    pdfDescription: '選擇 PDF 解析引擎，支援文字提取、圖片處理和表格識別',
    pdfProvider: 'PDF 解析器',
    pdfFeatures: '支援功能',
    pdfApiKey: 'API Key',
    pdfBaseUrl: 'Base URL',
    mineruDescription:
      'MinerU 是一個商用 PDF 解析服務，支援高階功能如表格提取、公式識別和佈局分析。',
    mineruApiKeyRequired: '使用前需要在 MinerU 官網申請 API Key。',
    mineruWarning: '注意',
    mineruCostWarning: 'MinerU 為商用服務，使用可能產生費用。請檢視 MinerU 官網瞭解定價詳情。',
    enterMinerUApiKey: '輸入 MinerU API Key',
    mineruLocalDescription:
      'MinerU 支援本地部署，提供高階 PDF 解析功能（表格、公式、佈局分析）。需要先部署 MinerU 服務。',
    mineruServerAddress: '本地 MinerU 伺服器地址（如：http://localhost:8080）',
    ...
    featureLayoutAnalysis: '佈局分析',
    featureMetadata: '後設資料',
    // Image Generation settings
    enableImageGeneration: '啟用 AI 圖片生成',
    imageGenerationDisabledHint: '啟用後，課程生成時將自動生成配圖',
    imageSettings: '影象生成',
    imageSection: '文生圖',
    imageProvider: '影象生成提供商',
    imageModel: '影象生成模型',
    providerSeedream: 'Seedream（位元組豆包）',
    providerQwenImage: 'Qwen Image（阿里通義）',
    providerNanoBanana: 'Nano Banana（Gemini）',
    providerGrokImage: 'Grok Image（xAI）',
    testImageGeneration: '測試影象生成',
    testImageConnectivity: '測試連線',
    imageConnectivitySuccess: '影象服務連線成功',
    imageConnectivityFailed: '影象服務連線失敗',
    imageTestSuccess: '影象生成測試成功',
    imageTestFailed: '影象生成測試失敗',
    imageTestPromptPlaceholder: '輸入影象描述進行測試',
    imageTestPromptDefault: '一隻可愛的貓咪坐在書桌上',
    imageGenerating: '正在生成影象...',
    imageGenerationFailed: '影象生成失敗',
    // Video Generation settings
    ...
    selectAgents: '選擇智慧體',
    noVisionWarning:
      '當前模型不支援視覺能力，圖片仍可放入幻燈片，但模型無法理解圖片內容來最佳化選擇和佈局',
    // Server provider configuration
    serverConfigured: '服務端',
    serverConfiguredNotice:
      '管理員已在服務端配置了此提供方的 API Key，可直接使用。也可輸入自己的 Key 覆蓋。',
    optionalOverride: '可選，留空則使用服務端配置',
    // Access code
    setupNeeded: '請先完成配置',
    ...
    clearCache: '清空本地快取',
    clearCacheDescription:
      '刪除所有本地儲存的資料，包括課堂記錄、對話歷史、音訊快取和應用配置。此操作不可撤銷。',
    clearCacheConfirmTitle: '確定要清空所有快取嗎？',
    clearCacheConfirmDescription: '此操作將永久刪除以下所有資料，且無法恢復：',
    clearCacheConfirmItems: '課堂和場景資料、對話歷史記錄、音訊和圖片快取、應用設定和偏好',
    clearCacheConfirmInput: '請輸入「確認刪除」以繼續',
    clearCacheConfirmPhrase: '確認刪除',
    clearCacheButton: '永久刪除所有資料',
    clearCacheSuccess: '快取已清空，頁面即將重新整理',
    clearCacheFailed: '清空快取失敗，請重試',
    // Web Search settings
    webSearchSettings: '網路搜尋',
    webSearchApiKey: 'Tavily API Key',
    webSearchApiKeyPlaceholder: '輸入你的 Tavily API Key',
    webSearchApiKeyPlaceholderServer: '已配置服務端金鑰，可選填覆蓋',
    webSearchApiKeyHint: '從 tavily.com 獲取 API Key，用於網路搜尋',
    webSearchBaseUrl: 'Base URL',
    webSearchServerConfigured: '服務端已配置 Tavily API Key',
    optional: '可選',
  },
  profile: {
    title: '個人資料',
