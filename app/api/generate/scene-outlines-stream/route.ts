...
export async function POST(req: NextRequest) {
  try {
    ...
    let availableImagesText =
      requirements.language === 'zh-CN' ? '无可用图片' : requirements.language === 'zh-TW' ? '無可用圖片' : 'No images available';
    ...
    const prompts = buildPrompt(PROMPT_IDS.REQUIREMENTS_TO_OUTLINES, {
      ...
      researchContext: researchContext || (requirements.language === 'zh-CN' ? '无' : requirements.language === 'zh-TW' ? '無' : 'None'),
      mediaGenerationPolicy,
      teacherContext,
    });
